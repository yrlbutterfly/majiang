...
def vllm_infer(
    ...
    for i in tqdm(range(0, len(train_dataset), batch_size), desc="Processing batched inference"):
        ...
        for j in range(len(batch["input_ids"])):
            ...
            # Extract image paths for this sample (best-effort)
            sample_images = []
            try:
                img_field = batch.get("images", None)
                if img_field is not None:
                    item = img_field[j]
                    if isinstance(item, list):
                        sample_images = [str(p) for p in item]
    ...
    # Ensure parent directory exists when save_name contains path components
    try:
    ...
    with open(save_name, "w", encoding="utf-8") as f:
        for idx, (text, pred, label) in enumerate(zip(all_prompts, all_preds, all_labels)):
            record = {"prompt": text, "predict": pred, "label": label, "images": all_images[idx] if idx < len(all_images) else []}
