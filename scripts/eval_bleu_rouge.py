...
def compute_metrics(sample):
    ...
    # accuracy（参考 src/llamafactory/train/sft/metric.py 的实现）
    label_digit = str(sample["label"]).strip()
    pred_clean = str(sample["predict"]).strip()
    is_correct = 1.0 if (pred_clean in {"0", "1", "2", "3"} and pred_clean == label_digit) else 0.0
    ...
    metric_result["accuracy"] = is_correct
