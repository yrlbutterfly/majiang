...
class ComputeSimilarity:
    ...
    def _dump(self) -> Optional[dict[str, float]]:
        ...
        self.score_dict = {"rouge-1": [], "rouge-2": [], "rouge-l": [], "bleu-4": [], "accuracy": []}
        return result
    ...
    def __call__(self, eval_preds: "EvalPrediction", compute_result: bool = True) -> Optional[dict[str, float]]:
        ...
        for pred, label in zip(decoded_preds, decoded_labels):
            # accuracy: 预测必须只包含一个与标签相同的数字（忽略标签中的\n）
            label_digit = label.strip()
            pred_clean = pred.strip()
            #is_correct = 1.0 if (pred_clean in {"0", "1", "2", "3", "4"} and pred_clean == label_digit) else 0.0
            is_correct = 1.0 if pred_clean == label_digit else 0.0
            #is_correct = 1.0 if (pred_clean in {"A", "B", "C"} and pred_clean == label_digit) else 0.0
        ...
        # l2_loss = np.mean(np.square(preds - labels))
